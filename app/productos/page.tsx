...
import { useCart } from '../components/CartContext'; // 👈 Importamos el carrito directo
...
export default function ProductosPage() {
  ...
  const { addItem } = useCart(); // 👈 Traemos la función de agregar
  ...
  return (
    <div className="bg-[#fcfbf9] min-h-screen font-sans text-neutral-800">
      <div className="max-w-7xl mx-auto px-6 pt-6">
        <Link href="/" className="inline-flex items-center text-sm font-semibold text-amber-800 hover:text-amber-900 transition">
          ← Volver al Inicio
        </Link>
      </div>

      <header className="py-10 px-6 text-center">
        <h1 className="text-4xl font-extrabold text-neutral-900 tracking-tight">Nuestro Catálogo</h1>
        <p className="mt-2 text-neutral-600 max-w-md mx-auto text-sm">Filtrá por categoría para encontrar tu outfit ideal.</p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2 max-w-2xl mx-auto px-4">
          {[
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-medium text-sm transition duration-200 shadow-sm ${categoriaActiva === cat.id ? 'bg-amber-800 text-white' : 'bg-white text-neutral-700 hover:bg-neutral-100 border border-neutral-200/60'}`}
            >
              {cat.nombre}
            </button>
          ))}
      ...
      <main className="max-w-7xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {filtrados.map((producto) => (
            <div key={producto.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-neutral-100 p-4 group hover:shadow-md transition duration-300">
              <div className="h-64 bg-neutral-100 rounded-xl overflow-hidden relative">
                <img src={producto.imagen} alt={producto.nombre} className="w-full h-full object-cover group-hover:scale-103 transition duration-500" />
              </div>
              <div className="mt-4">
                <h3 className="font-bold text-base text-neutral-900 group-hover:text-amber-800 transition">{producto.nombre}</h3>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-3">
                  <div>
                    ...
                    <span className="text-lg font-extrabold text-neutral-900">${producto.precio.toLocaleString('es-AR')}</span>
                  </div>
                  <button 
                    onClick={() => addItem(producto)} // 👈 ¡Botón conectado de forma directa y limpia!
                    className="bg-neutral-900 hover:bg-amber-800 text-white text-xs font-semibold px-4 py-2.5 rounded-xl transition cursor-pointer"
                  >
                    Agregar
                  </button>
                </div>
        ...
        {filtrados.length === 0 && <p className="text-center text-neutral-500 mt-12">No hay productos en esta categoría por el momento.</p>}
      </main>
    </div>
  );
}